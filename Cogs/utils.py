...
class Utils(commands.Cog, name = 'Utils'):
	...
	@commands.command(brief = 'True', description = '**Устанавливает анти-рейд режым. Есть не сколько режымов, 1 - Слабый 5сек задержка в текстовых каналах и средний уровень модерации, 2 - Сильний 10сек задержка в текстовых каналах и високий уровень модерации, 3 - Найвисшый 15сек задержка в текстовых каналах и найвисшый уровень модерации**', usage = 'anti-rade [Время действия] [Уровень защиты]')
	...
	async def voicechannel( self, ctx, state: str ):
		...
		if state in on_answers:
			...
			if 'channel_id' not in data:
				voice_channel = await ctx.guild.create_voice_channel( "Нажми на меня", category = channel_category )
	...
	async def serverstats( self, ctx, stats_type, stats_count ):
		...
		stats_channel = await ctx.guild.create_voice_channel(f'[{counters[stats_count.lower()][1]}] {counters[stats_count.lower()][0]}', category = stats_category )
