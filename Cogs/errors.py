...
class Errors(commands.Cog, name = 'Errors'):
	...
	async def on_command_error(self, ctx, error):
		...
		if isinstance( error, commands.errors.CommandOnCooldown ):
			...
			await ctx.message.add_reaction('❌')
			emb = discord.Embed(title='Ошибка!', description=f'**Вы не указали аргумент. Укажити аргумент - {error.param.name} к указаной команде!**\n\n{ctx.command.help.format(Prefix=PREFIX)}', colour=discord.Color.green())
			emb.set_author(name=ctx.author.name, icon_url=ctx.author.avatar_url)
			emb.set_footer(text=self.FOOTER, icon_url=self.client.user.avatar_url)
			await ctx.send(embed=emb)
			...
			emb = discord.Embed(title='Ошибка!', description=f'**Указан не правильный аргумент!**\n\n{ctx.command.help.format(Prefix=PREFIX)}', colour=discord.Color.green())
			emb.set_author(name=ctx.author.name, icon_url=ctx.author.avatar_url)
			emb.set_footer(text=self.FOOTER, icon_url=self.client.user.avatar_url)
			await ctx.send(embed=emb)
