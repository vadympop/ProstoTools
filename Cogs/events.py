...
class Events(commands.Cog, name = 'Events'):
	...
	async def on_member_update(self, before, after):
		...
		if not len(before.roles) == len(after.roles):
			...
			e = discord.Embed(description=f'У пользователя `{str(after)}` были изменены роли', colour=discord.Color.green(), timestamp=datetime.datetime.utcnow())
			e.add_field(name="Было сделано", value=f"**{''.join(roles)}**", inline=False)
			e.add_field(name="Id Участника", value=f"`{after.id}`", inline=False)
			e.set_author(name='Журнал аудита | Изменение ролей участника', icon_url=before.avatar_url)
			e.set_footer(text=self.FOOTER, icon_url=self.client.user.avatar_url)
			await channel.send(embed=e)

		if not before.display_name == after.display_name:
			e = discord.Embed(description=f'Пользователь `{str(before)}` изменил ник', colour=discord.Color.green(), timestamp=datetime.datetime.utcnow())
			e.add_field(name="Действующее имя", value=f"`{after.display_name+'#'+after.discriminator}`", inline=False)
			e.add_field(name="Предыдущее имя", value=f"`{before.display_name+'#'+before.discriminator}`", inline=False)
			e.add_field(name="Id Участника", value=f"`{after.id}`", inline=False)
			e.set_author(name='Журнал аудита | Изменение ника участника', icon_url=before.avatar_url)
			e.set_footer(text=self.FOOTER, icon_url=self.client.user.avatar_url)
			await channel.send(embed=e)
	...
	async def on_member_ban(self, guild, user):
		...
		e = discord.Embed(description=f'Пользователь `{str(user)}` был забанен', colour=discord.Color.green(), timestamp=datetime.datetime.utcnow())
		e.add_field(name="Причина бана", value=f"""`{'Причина не указана' if not ban.reason else ban.reason}`""", inline=False)
	...
	async def on_member_unban(self, guild, user):
		...
		if channel_id == 0:
			return
		channel = self.client.get_channel(channel_id)
		e = discord.Embed(description=f'Пользователь `{str(user)}` был разбанен', colour=discord.Color.green(), timestamp=datetime.datetime.utcnow())
		e.add_field(name="Id Участника", value=f"`{user.id}`", inline=False)
		e.set_author(name='Журнал аудита | Разбан пользователя', icon_url=user.avatar_url)
		e.set_footer(text=self.FOOTER, icon_url=self.client.user.avatar_url)
		await channel.send(embed=e)			
	...
	async def on_message_delete(self, message):
		...
		if channel_id == 0:
			return
		channel = self.client.get_channel(channel_id)
		e = discord.Embed(colour=discord.Color.green(), timestamp=datetime.datetime.utcnow())
		e.add_field(name="Удалённое сообщение", value=f"```{message.content}```" if message.content else 'Сообщения отсутствует ', inline=False)
		e.add_field(name="Автор сообщения", value=f"`{str(message.author)}`", inline=False)
		...
		e.set_author(name='Журнал аудита | Удаление сообщения', icon_url=message.author.avatar_url)
		e.set_footer(text=self.FOOTER, icon_url=self.client.user.avatar_url)
		await channel.send(embed=e)
	...
	async def on_message_edit(self, before, after):
		...
		if channel_id == 0:
			return
		channel = self.client.get_channel(channel_id)
		e = discord.Embed(description=f'**[Сообщение]({before.jump_url}) было изменено**', colour=discord.Color.green(), timestamp=datetime.datetime.utcnow())
		e.add_field(name="Старое содержимое", value=f"```{before.content}```", inline=False)
		e.add_field(name="Новое соодержиое", value=f"```{after.content}```", inline=False)
		...
		e.set_author(name='Журнал аудита | Изменение сообщения', icon_url=before.author.avatar_url)
		e.set_footer(text=self.FOOTER, icon_url=self.client.user.avatar_url)
		await channel.send(embed=e)
