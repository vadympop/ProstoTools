...
global Footer, Background, Font, Save
Footer = configs['FOOTER_TEXT']
Background = configs['DEF_PROFILE_BG']
Font = configs['FONT']
Save = configs['SAVE_IMG']
...
class Economy(commands.Cog, name = 'Economy'):
	...
	async def top(self, ctx):
		...
		emb_rep.set_footer( text = Footer, icon_url = client.user.avatar_url )
		...
		emb_exp.set_footer( text = Footer, icon_url = client.user.avatar_url )
		...
		emb_lvl.set_footer( text = Footer, icon_url = client.user.avatar_url )
		...
		emb_money.set_footer( text = Footer, icon_url = client.user.avatar_url )
		...
		emb_coins.set_footer( text = Footer, icon_url = client.user.avatar_url )
		...
		emb_msg.set_footer( text = Footer, icon_url = client.user.avatar_url )
	...
	@commands.command(name = 'text-channel', description = '**Создает приватный текстовый канал. По умолчанию у вас есть 20 каналов(Их можно купить в магазине), создавать их можно только в определлёной категории. Он автоматически удаляеться через 30мин!(Cooldown - 3 мин)**', usage = 'text-channel [Имя канала]')
			...
			def buy_item( item, cost, stacked = False ):
				...
				if item not in member_items:
					...
					if stacked == True:
						...
						if stack == False:
							...
							if state == True:
								...
								emb_prison.set_footer( text = Footer, icon_url = client.user.avatar_url )
							...
							emb_cool.set_footer( text = Footer, icon_url = client.user.avatar_url )
					...
					if stacked == False:
						...
						emb_fail.set_footer( text = Footer, icon_url = client.user.avatar_url )
	...
	@commands.command(name = 'send-money', description = '**Этой командой можно оправить свои деньги другому пользователю(Cooldown - 30 мин)**', usage = 'send-money [@Участник]')
	...
	@commands.command(hidden = True, name = 'remove-role', description = '**Удаляет указаную роль из профиля пользователя(Объязательно используйте эту команду для снятия роли, а не простое удаление роли через сам дискорд!, Cooldown - 3 часа)', usage = 'remove-role [@Участник] [@Роль]')
	...
	@commands.command(hidden = True, name = 'add-cash', description = '**Добавляет указаний тип валюты в профиль**', usage = 'add-cash [@Участник] [Название валюты] [Количество]')
	@commands.has_permissions( administrator = True )
	@commands.cooldown(1, 14400, commands.BucketType.member)
	...
	@commands.command(hidden = True, name = 'remove-cash', description = '**Удаляет указаний тип валюты из профиля**', usage = 'remove-cash [@Участник] [Название валюты] [Количество]')
	@commands.has_permissions( administrator = True )
	@commands.cooldown(1, 14400, commands.BucketType.member)
	...
	async def profile( self, ctx, member: typing.Optional[discord.Member] = None ):
		...
		if not member:
			...
			if not user_profile:  
				...
				img = Image.open(Background[:-8]+f"{user_profile}.png")
			...
			bigtext = ImageFont.truetype( Font, size = 34 )
			midletext = ImageFont.truetype( Font, size = 23)
			...
			idraw.text((15, 245), Footer, font = midletext)

			img.save(Save)
			await ctx.send( file = discord.File( fp = Save ) )
